...
class Menu extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            [['route', 'data', 'parent','order'], 'default'],
    ...
    {
        $cats       = self::find()->asArray()->all();
        return $cats ;
    }

    public function getCatsTree($cats, $bclassid=null, $nu=0)
    {
        $bx             = '---|' ;
        $nu++ ;
        foreach ($cats as $cat) {
            ...
            $catbid     = $cat['parent'] ;
            if ($catbid == $bclassid) {
                self::$tree[$catid] = str_repeat($bx, $nu) . '~' . $catname . PHP_EOL ;
                $this->getCatsTree($cats, $catid, $nu) ;
            }
        }
    }
